...
/**
 * This class provides a small example application to generate JWT tokens for the dynamic log level feature. The input
 * data is hard-coded in the source-code. Please change the value for your use-case.
 */
public class TokenCreator {
    ...
    /**
     * Run this application locally in order to generate valid dynamic log level JWT tokens which enable you to change
     * the log level threshold on your CF-Application for a single thread.
     */
    public static void main(String[] args)
            throws NoSuchAlgorithmException, NoSuchProviderException, DynamicLogLevelException,
        ...
        System.out.println(
                "You successfully created a dynamic log level token with log level " + level + " and packages " + packages + "!");
        System.out.println();
    ...
    public static String createToken(KeyPair keyPair, String issuer, Date issuedAt, Date expiresAt, String level,
        ...
        if (ALLOWED_DYNAMIC_LOGLEVELS.contains(level)) {
            ...
            throw new DynamicLogLevelException(
                    "Dynamic Log-Level [" + level + "] provided in header is not valid. Allowed Values are " + ALLOWED_DYNAMIC_LOGLEVELS.toString());
        }
    }
