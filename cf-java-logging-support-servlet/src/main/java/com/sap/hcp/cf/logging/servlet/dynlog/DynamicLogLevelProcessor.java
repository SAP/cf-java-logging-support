...
/**
 * This class provides a mechanism that reads a token from an HTTP-request-header. If this token is provided and does
 * contain a correct signature, valid timestamps and a log-level-value, the log-level for the thread triggered by this
 * request will be changed to the provided value.
 *
...
public class DynamicLogLevelProcessor {
    ...
    private static final List<String> ALLOWED_DYNAMIC_LOGLEVELS =
            Arrays.asList("TRACE", "DEBUG", "INFO", "WARN", "ERROR");
    private final TokenDecoder tokenDecoder;

    /**
     * @param dynLogConfig
     *         the {@link DynamicLogLevelConfiguration} to read the public RSA key for JWT validation from.
     * @deprecated Use {@link DynamicLogLevelProcessor#DynamicLogLevelProcessor(RSAPublicKey)} instead.
     */
    @Deprecated
    ...
    /**
     * Decodes and validate the JWT. Configures the dynamic log levels by setting the corresponding fields in the MDC.
     *
     * @param logLevelToken
    ...
    /**
     * Extracts the relevant claims for dynamic log level configuration from the decoded token. In case of faulty
     * content, i.e. unknown log level a {@link DynamicLogLevelException} is thrown. You can override this method to
     * implement own interaction with the JWT, e.g. extraction and validation of additional claims.
     *
     * @param jwt
    ...
    protected void processJWT(DecodedJWT jwt) throws DynamicLogLevelException {
        ...
        if (ALLOWED_DYNAMIC_LOGLEVELS.contains(dynamicLogLevel)) {
            ...
            throw new DynamicLogLevelException(
                    "Dynamic Log-Level [" + dynamicLogLevel + "] provided in header is not valid. Allowed Values are " + ALLOWED_DYNAMIC_LOGLEVELS.toString());
        }
    }

    /**
     * Resets the current dynamic log level configuration by removing the corresponding fields from the MDC. This needs
     * to be called to remove the changed log level configuration.
     */
